...
class War:
    ...
    async def attack(self, user: UserData, user_vk_id: int, enemy: UserData, enemy_vk_id: int):
        ...
        battle = await Battle.create(attack=user.user, defender=enemy.user, attack_vk_id=user_vk_id, defender_vk_id=enemy_vk_id, time=int(datetime.datetime.utcnow().timestamp()), win=win, data=data, reward=reward)
